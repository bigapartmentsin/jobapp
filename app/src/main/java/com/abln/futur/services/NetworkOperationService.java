package com.abln.futur.services;
...
import com.abln.futur.common.GAConstants;
...
import static com.abln.futur.common.NetworkConfig.API_URL;
...
//            }
            Retrofit mRetrofit = new Retrofit.Builder()
                    ...
                    .client(FuturApiClient.getClient())
                    .build();
...
//            int apiUrlType = intent.getIntExtra(NetworkConfig.SESSION_TYPE, 0);
    ...
    private void sendBroadcastToUI(String apiUrl, String responseString, Intent requestIntent) {
        ...
        //Send broadcast
        FLog.d(TAG, "doNetworkOperation():: sendBroadcastToUI()");
    ...
    private class BackgroundTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            if (mIntent == null) {
                FLog.e(TAG, "doInBackground() intent is null !!");
                return null;
            }
//            doGetCall();
            doNetworkOperation();
        ...
        protected Void doNetworkOperation() {
            ...
            if (apiUrl == null ||
                    ...
                    headerMap == null ||
                    inputBody == null) {
                FLog.e(TAG, "doNetworkOperation():: some values is null !!");
            }
            FLog.d(TAG, "doNetworkOperation():: Request Url : " + apiUrl + " \nBody : " + DataParser.toJsonString(inputBody));
...
//            }
            ...
            networkCall.enqueue(new Callback<Object>() {
                @Override
                public void onResponse(Call<Object> call, Response<Object> response) {
                    FLog.d(TAG, "================> " + NetworkConfig.BASE_URL + apiUrl + "    " + (System.currentTimeMillis() - timeStamp) + " ms");
                    if (response.body() != null) {
                        try {
                            logForGoogleAnalytics(timeStamp, apiUrl, inputBody, null);
                            String jsonResponse = response.body().toString();
                            FLog.d(TAG, "doNetworkOperation():: onResponse() setting Body" + jsonResponse);
                            Gson gson = new Gson();
