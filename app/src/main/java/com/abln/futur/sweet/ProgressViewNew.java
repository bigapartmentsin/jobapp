...
public class ProgressViewNew extends LinearLayout {

    private static final String TAG = ProgressViewNew.class.getSimpleName();
    ...
    private final List<PausableBar> progressBars = new ArrayList<>();
    ...
    public ProgressViewNew(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public ProgressViewNew(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ProgressViewNew(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, jp.shts.android.storiesprogressview.R.styleable.StoriesProgressView);
        storiesCount = typedArray.getInt(jp.shts.android.storiesprogressview.R.styleable.StoriesProgressView_progressCount, 0);
        typedArray.recycle();
        bindViews();
    }
    ...
    private void bindViews() {
        ...
        for (int i = 0; i < storiesCount; i++) {
            final PausableBar p = createProgressBar();
            progressBars.add(p);
            addView(p);
            if ((i + 1) < storiesCount) {
                addView(createSpace());
    ...
    private PausableBar createProgressBar() {
        PausableBar p = new PausableBar(getContext());
    ...
    public void skip() {
        ...
        if (current < 0) return;
        PausableBar p = progressBars.get(current);
        isSkipStart = true;
        p.setMax();
    }
    ...
    private PausableBar.Callback callback(final int index) {
        return new PausableBar.Callback() {
            @Override
            ...
            public void onFinishProgress() {
                if (isReverseStart) {
                    ...
                    if (0 <= (current - 1)) {
                        PausableBar p = progressBars.get(current - 1);
     ...
     */
    public void destroy() {
        for (PausableBar p : progressBars) {
            p.clear();
        }
    }
